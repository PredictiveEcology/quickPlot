...
vignette: >
  ...
  %\VignetteDepends{ggplot2, igraph, RColorBrewer, quickPlot}
...
library(raster)
library(quickPlot)
filelist <- data.frame(file = (system.file("maps", package = "quickPlot") %>%
  dir(., full.names = TRUE, pattern = "tif"))[-c(2, 5) ], stringsAsFactors = FALSE)  
       # omit forestAge and percentPine for simplicity
print(filelist)
# Load files to memory (using rasterToMemory), assign to a simList we call maps 
maps <- lapply(filelist$file, function(x) raster(x))
names(maps) <- sapply(basename(filelist$file), function(x) strsplit(x, split="\\.")[[1]][1])
...
`Plot` methods can be extended to accommodate other classes. See package `SpaDES` for examples. 
...
```{r Plot a .quickPlot object, eval=FALSE, echo=TRUE, cache=TRUE}
clearPlot()
